...
<div class="row container" id="wrapper">
    ...
    <main id="main-contents" class="col-xs-12 col-sm-12 col-md-8">
       <section>
          ...
          <div class="halim_box">
            @foreach ($category_home as $cate)
               @foreach ($cate->movie as $cate_mov)
                  <article class="col-md-3 col-sm-3 col-xs-6 thumb grid-item post-27021">
                     <div class="halim-item">
                        <a class="halim-thumb" href="{{ route('movie',$cate_mov->id) }}" title="{{ $cate_mov->tittle }}">
                           <figure><img class="lazy img-responsive" src="{{ asset('uploads/movie/'.$cate_mov->image) }}" alt="{{ $cate_mov->tittle }}" title="{{ $cate_mov->tittle }}"></figure>
                           <span class="status">5/5</span><span class="episode"><i class="fa fa-play" aria-hidden="true"></i>Vietsub</span> 
                           <div class="icon_overlay"></div>
                           <div class="halim-post-title-box">
                              <div class="halim-post-title ">
                                 <p class="entry-title">{{ $cate_mov->tittle }}</p>
